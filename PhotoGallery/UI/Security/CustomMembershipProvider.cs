...
namespace UI.Security
{
    public class CustomMembershipProvider : MembershipProvider
    {
        ...
        public MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status, string OpenID)
        {
            ValidatePasswordEventArgs args = new ValidatePasswordEventArgs(username, password, true);
            OnValidatingPassword(args);
            if (args.Cancel)
            ...
            {
                UserRepository UserRepository = new UserRepository();
                UserRepository.CreateUser(username, password, email, OpenID);
                status = MembershipCreateStatus.Success;
                return GetUser(username, false);
            }
            else
        ...
        {
            UserRepository UserRepository = new UserRepository();
            return UserRepository.GetUser(OpenID);
        }
        ...
        {
            if (UserBLLService.GetUserByLogin(username).UserPassword == password)
            {
                return true;
            }
            else
